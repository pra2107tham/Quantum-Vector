...
const webinarData: Record<string, any> = {
  "devops-live-april-may-2026": {
    ...
    projectHighlight: {
      ...
      description:
        "A complete real-time microservices platform deployed end-to-end on Kubernetes — covering every layer from containerization to observability.",
      services: [
        "Node.js (Express) microservice — REST API backend",
        "Python (Flask) microservice — lightweight service layer",
        "Spring Boot (Java) microservice — enterprise-grade backend service",
        "React.js + Nginx frontend — UI served via Nginx on Kubernetes",
        "MongoDB (StatefulSet) — persistent database on Kubernetes",
        "Redis (cache) — in-cluster caching layer",
        "RabbitMQ — asynchronous messaging and event-driven communication",
        "Nginx Ingress Controller — HTTP/HTTPS routing and API gateway",
        "Prometheus + Grafana — full metrics and dashboard observability stack",
        "Alertmanager — Slack/Email alerting for production incidents",
      ],
    },
    whyMatters: {
    ...
    projects: [
      "Day 1: Docker fundamentals — Dockerfiles, multi-stage builds, image optimization",
      "Day 1: Containerize Node.js, Python Flask, and Spring Boot microservices",
      "Day 2: Set up a Kubernetes cluster on-premises using kubeadm + containerd + Flannel CNI",
      "Day 2: Deploy all microservices to Kubernetes — Pods, Deployments, Services (ClusterIP/LB)",
      "Day 3: Deploy MongoDB as a StatefulSet, Redis as a cache layer, and RabbitMQ for messaging",
      "Day 3: Configure Nginx Ingress Controller and manage deployments with Helm 3",
      "Day 4: Set up Prometheus (Node Exporter, kube-state-metrics) and Grafana dashboards",
      "Day 4: Configure Alertmanager with Slack/Email alerts and wire up GitHub Actions + ArgoCD CI/CD",
    ],
    outcomes: [
      "Containerize multi-language microservices with Docker and multi-stage builds",
      "Provision and manage a Kubernetes cluster on-premises using kubeadm",
      "Deploy stateful workloads (MongoDB, Redis) and messaging systems (RabbitMQ) on Kubernetes",
      "Manage Kubernetes applications with Helm 3 charts",
      "Build a full Prometheus + Grafana observability stack with real-time dashboards",
      "Configure Alertmanager to trigger Slack/Email alerts for production incidents",
      "Implement a CI/CD pipeline with GitHub Actions and ArgoCD GitOps",
      "Deploy a complete real-time microservices project end-to-end on AWS EC2",
    ],
    whatYoullLearn: [
    ...
    whatYoullGet: [
      ...
      "Complete microservices project source code (Node.js, Python, Java, React)",
      "Kubernetes manifests and Helm charts for all services",
      "Prometheus + Grafana dashboard configurations and Alertmanager setup",
      "GitHub Actions CI/CD pipeline configs and ArgoCD GitOps setup",
